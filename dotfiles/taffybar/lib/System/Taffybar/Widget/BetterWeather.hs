...
module System.Taffybar.Widget.BetterWeather
  ( WeatherConfig(..)
  , WeatherInfo(..)
...
defaultWeatherFormatter :: StringTemplate T.Text -> WeatherInfo -> T.Text
defaultWeatherFormatter tpl wi = render tpl'
  where
    ...
    Right wi ->
      case formatter of
        DefaultWeatherFormatter -> do
          let rawLabel = defaultWeatherFormatter labelTpl wi
          let rawTooltip = defaultWeatherFormatter tooltipTpl wi
...
data WeatherFormatter
  = WeatherFormatter (WeatherInfo -> IO (T.Text, Maybe T.Text)) -- ^ Specify a custom formatter for 'WeatherInfo'
